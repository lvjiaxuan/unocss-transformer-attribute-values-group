...
var src_default = {
  ...
  transform(code) {
    ...
    const str = code.toString().replace(
      ...
      (from, pre, values) => values.split(/\s/g).filter(Boolean).map((v, i, a) => `${pre + v}${i === a.length - 1 ? "" : "]"}`).join(",")
    ).replace(
      dataAttributeValuesGroupRegex,
      (from, pre, values, variant) => values.split(/\s/g).filter(Boolean).map((i) => `${pre}${i}]:${variant.replace(/[\n\r]?/g, "").replace(/ {2,}/g, " ")}`).join(" ")
    );
